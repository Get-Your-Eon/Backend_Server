...
from app.services.station_service_kepco_backup import StationService
from app.schemas.station import StationSummary, StationDetail, ErrorResponse

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/kepco-stations", tags=["KEPCO Stations"])


@router.get(
    "",
    response_model=List[StationSummary],
    summary="Search KEPCO charging stations by location",
    description="""
    Search for EV charging stations using KEPCO API within a specified radius of given coordinates.
...
@router.get(
    ...
    summary="Get KEPCO charger details for a station",
    description="""
    Get detailed information about all chargers at a specific KEPCO charging station.
...
@router.get(
    "/{cs_id}",
    response_model=StationDetail,
    summary="Get KEPCO station details (alias for /chargers)",
    description="Alias endpoint for getting KEPCO station charger details"
)
